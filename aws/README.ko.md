...
AWS 워크숍은 S3 storage와 observability를 로컬에서 먼저 검증할 수 있는 예제로
...
모든 모듈은 기본 학습 경로를 local-first로 유지합니다. S3 모듈은 로컬 AWS 호환
인프라를 사용하고, CloudWatch/IMDS 모듈은 local adapter bean을 사용하므로 기본 테스트는
실제 AWS 서비스나 IMDS를 호출하지 않습니다.
...
| AWS account | 로컬 워크숍 경로에는 필요하지 않습니다. 실제 CloudWatch/IMDS 동작은 수동 opt-in입니다. |
