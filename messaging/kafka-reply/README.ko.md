...
이 예제는 **Kafka Reply Demo** 모듈을 실행 가능한 메시지 기반 워크플로우 예제로 보여줍니다. 개발자가 먼저 확인할 경로인 모듈 설정, 샘플 또는 테스트 실행, 반복적인 인프라 코드를 줄이는 라이브러리 또는 프레임워크 API 사용 방식을 중심으로 설명합니다.
...
1. `messaging-kafka-reply` 예제에 필요한 로컬 런타임을 준비합니다.
