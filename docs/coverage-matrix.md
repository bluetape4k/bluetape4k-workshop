...
| `bluetape4k-exposed-ktor` | `ktor/exposed-rest` | ✅ Good | legacy aggregator가 core/backend 경계를 함께 노출했고 Ktor consumer가 그 경계에 묶여 있었음 | backend-neutral core health/readiness와 선택형 JDBC adapter | R2DBC/cache adapter를 classpath 조건으로 추가하는 Ktor consumer fixture | #880 |
...
| `bluetape4k-javers` / `bluetape4k-javers-persistence-redis` / `bluetape4k-javers-persistence-kafka` | `exposed-javers-approval-workflow`, `exposed-javers-persistence-audit` | ✅ Good | 실행 중 외부 head 변조 감지, multi-partition ordering, Redis post-EXEC commit-unknown 자동 복구, cursor pagination과 filtered query pushdown은 범위 밖 | `QueryBuilder.limit` 기반 `1..100` approved history와 newest-first ordering | Exact-instance Redisson read/decode bound, malformed/snapshot-only Redis head startup fail-closed, single-partition Kafka→Lettuce projection | #892, #893, #894 |
...
| `bluetape4k-spring-boot-redis` / `bluetape4k-virtualthread-api` | `spring-boot-cache-redis` | ✅ Good | custom codec, key별 TTL과 강제 drain timeout은 범위 밖 | Redis L2 cache와 Spring-owned shared executor | provider/fallback 선택, Lettuce→adapter→delegate 종료 순서, MDC 복원 | #940 |
...
| `bluetape4k-nats` JetStream Consumer Flow | `messaging-nats-jetstream-flow` | ✅ Good | 운영 cluster의 TLS/auth, durable offset 외부 저장소와 exactly-once 처리는 범위 밖 | `ConsumerContext` pull과 `JetStream` push cold Flow, bounded capacity/pending limit | Testcontainers redelivery, caller `ack`/`nak`/`term`, cancellation cleanup과 실제 pending-drop fail-fast | #923 |
