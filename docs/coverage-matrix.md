...
| `bluetape4k-images` / `bluetape4k-images-spring-boot` | `image-processing/advanced-workflow`, `image-processing/ocr-api`, `image-processing/profile-image-moderation` | ✅ Good | 기존 workflow가 object metadata를 body download 없이 확인하지 않았음 | `ImageObjectMetadataReader` capability, Local stat/S3 HEAD metadata, bounded multi-page TIFF structured OCR, unsupported fallback과 race fail-closed | private original, blurred pending image, default fallback을 포함한 image workflow | #883, #884 |
