...
초기 조사에서 upstream README가 안내한 external AspectJ CTW 경로를 검토했지만,
consumer source build와 artifact smoke 모두 `LeaderElectionAspect.aspectOf()` 초기화
중 `NoAspectBoundException`과 존재하지 않는 no-arg constructor의
`NoSuchMethodError`를 재현했다. 따라서 이 사양의 최종 실행 모델은
`@EnableAspectJAutoProxy(proxyTargetClass = true)`와 `spring.aop.auto=false`를
사용하는 Spring runtime proxy다. 아래에 남아 있는 CTW/weaving 표현과 해당 acceptance는
당시 조사 기록이며 최종 구현 계약을 덮어쓰지 않는다. Freefair plugin이나 external
singleton weaving은 이 consumer 범위에서 사용하지 않는다.
...
| `gradle/libs.versions.toml:5,143-144,180,228` | `bluetape4k-dependencies-version = "2.0.0-SNAPSHOT"`이고 `bluetape4k-leader-spring-boot`와 `bluetape4k-leader-micrometer`는 BOM이 버전을 결정하는 versionless alias다. |
...
- `leader/tenant-scheduler`의 versionless `bluetape4k-leader-spring-boot`·`bluetape4k-leader-micrometer` consumer dependency 추가
...
alias는 이미 `gradle/libs.versions.toml`에 있고, root BOM이 `2.0.0-SNAPSHOT` 버전을 공급한다. `leader/backend-comparison-lab`이 같은 alias를 사용하는 현재 저장소 패턴도 확인했다. 개별 Bluetape4k 버전이나 별도 BOM은 선언하지 않는다.

upstream README의 external CTW 경로도 확인했지만, 현재 `leader-spring-boot` 시험판 artifact의 `LeaderElectionAspect.aspectOf()`가 존재하지 않는 no-arg constructor를 호출해 `NoAspectBoundException`을 내는 것을 source build와 consumer smoke에서 재현했다. 이 consumer 예제는 깨진 external singleton 경로를 복제하지 않고, Spring runtime proxy로 실제 동작을 검증한다. 따라서 Freefair CTW plugin은 이 모듈의 dependency graph에 추가하지 않는다.

### 2. profile 격리

`leader/tenant-scheduler/src/main/kotlin/io/bluetape4k/workshop/leader/tenantscheduler/scheduled/TenantScheduledPolicyConfiguration.kt`에 `TenantScheduledPolicyConfiguration`을 `@Profile("scheduled-policy")`, `@Configuration(proxyBeanMethods = false)`, `@EnableScheduling`, `@EnableAspectJAutoProxy(proxyTargetClass = true)`로 만든다. configuration은 bean 이름이 `tenantScheduledPolicyFixture`인 main-source fixture를 하나 제공한다. CGLIB 기반 Spring proxy가 적용될 수 있도록 fixture는 `open class`와 `open fun`으로 선언한다. `reconcile`은 `@Scheduled(fixedDelay = 5_000, initialDelay = 60_000)`인 plain scheduled method로 남겨 property policy가 실제로 적용되는 경로를 보인다. YAML의 `min-lease-time: 5s`를 local elector가 보장하는 동안 호출이 대기할 수 있으므로 유효한 시작 간격은 fixed delay보다 길어질 수 있으며, README와 테스트에서 이 blocking 경계를 설명한다. 긴 `initialDelay`는 예제 기동 직후의 비결정적인 반복 실행을 막는다. `spring.aop.auto=false`는 이 명시적인 proxy creator와 Boot auto proxy creator의 중복을 막는다.
...
이 consumer 실행 모델에서는 profile configuration이 `@EnableAspectJAutoProxy(proxyTargetClass = true)`로 upstream `LeaderElectionAspect` bean을 한 번만 proxy한다. `spring.aop.auto=false`가 Boot의 자동 proxy creator를 끄므로 직접 호출과 Spring scheduler callback 모두 같은 proxy 경로를 사용한다. 반대로 `bluetape4k.leader.aop.enabled=false`이면 factory 조건이 꺼져 policy registry/BPP와 aspect bean이 구성되지 않을 수 있고, profile configuration의 Spring scheduled task만 남는다. 이 경우 task가 leader ownership 없이 실행될 수 있으므로 README에 단일 프로세스 학습용 opt-out 경계를 경고하고 context test에서 proxy creator, registry/task 조건, 실제 proxy invocation을 각각 확인한다. 기본 profile과 `scheduled-policy` profile은 이 값을 끄지 않는다.

### 5. 테스트 구성

`leader/tenant-scheduler/src/test/kotlin/io/bluetape4k/workshop/leader/tenantscheduler/scheduled/TenantScheduledPolicyContextTest.kt`와 `TenantScheduledPolicyLifecycleTest.kt`를 추가하고, upstream 테스트와 같은 `ApplicationContextRunner` 패턴을 사용한다. profile binding 테스트는 `ConfigDataApplicationContextInitializer`와 `spring.profiles.active=scheduled-policy`를 사용해 실제 `application-scheduled-policy.yml`을 읽는다. inline property는 malformed/edge-case와 failure assertion에만 사용해 파일과 테스트 계약이 서로 우회되지 않게 한다. proxy invocation은 test sourceSet fixture로 흉내 내지 않고 `open`으로 선언한 main-source `tenantScheduledPolicyFixture`를 실제 context에서 호출하는 `@SpringBootTest` smoke test로 검증한다. 이 smoke는 packaged YAML의 `min-lease-time=5s` floor를 그대로 실행하므로 두 direct call을 합쳐 `assertTimeout(Duration.ofSeconds(15))` 상한을 둔다. scheduler trigger와 edge-case runner만 `min-lease-time=0s` override를 사용해 신호와 semantic failure를 bounded하게 확인한다. 이 테스트는 `AopUtils.getTargetClass`와 leader factory/leader-aspect observation 경로를 확인하고, `internalAutoProxyCreator`가 존재하며 proxy target이 main-source fixture인지 고정한다. 직접 bean 호출은 Spring `ScheduledMethodRunnable` wrapper를 우회하므로 Spring scheduler-level Observation을 이 smoke의 성공 조건으로 주장하지 않는다. scheduler task 등록과 close lifecycle은 별도 `ScheduledTaskHolder` 검증으로 고정한다. `ApplicationContextRunner`는 registry/task binding과 조건부 auto-configuration 검증에 한정한다. overload method의 registry signature 검증은 별도 unit test로 두어 Spring의 “scheduled method는 인자를 받지 않는다” 검증과 policy selector 모호성 검증을 구분한다.

| 검증 | 기대 결과 |
|---|---|
| 기본 또는 `scheduled-policy` 미활성 | registry/BPP와 `tenantScheduledPolicyFixture` task가 없고, 기존 retention task 같은 unrelated Spring task는 유지될 수 있으며 기존 19개 reducer test가 그대로 통과한다. |
...
| proxy invocation + leader-aspect observation | main-source `tenantScheduledPolicyFixture.reconcile()`를 context bean에서 직접 두 번 호출해 Spring proxy, local factory 선택, leader-aspect observation 경로가 동작함을 확인하고, `ScheduledTaskHolder` 등록은 정확히 1개로 유지한다. packaged YAML의 `min-lease-time=5s` floor를 그대로 실행하며 두 호출 합계에 `assertTimeout(Duration.ofSeconds(15))`를 적용한다. scheduler trigger와 edge-case runner만 `min-lease-time=0s`를 사용한다. 이 직접 호출은 Spring `ScheduledMethodRunnable` wrapper를 거치지 않으므로 scheduler-level Observation은 이 행의 acceptance 범위가 아니며, task 등록·trigger·close는 별도 lifecycle assertion으로 검증한다. context close와 fixture 상태 정리는 `finally`에서 수행한다. production fixture의 `initialDelay = 60_000` 때문에 자동 첫 tick을 기다리지 않아도 되며 직접 호출은 scheduler 재실행과 경쟁하지 않는다. |
| Spring proxy/AOP 경계 | `spring.aop.auto=false`에서는 명시적인 Spring proxy creator가 한 번만 적용되고, `bluetape4k.leader.aop.enabled=false`에서는 factory/registry/aspect 조건이 빠져 task만 남을 수 있다. `internalAutoProxyCreator` 존재와 proxy invocation을 main-source smoke test가 확인한다. |
| README snippet | 기존 `TenantSchedulerReadmeSnippetTest`가 계속 통과한다. |

테스트는 시간 경쟁을 피하도록 lifecycle fixture에 `initialDelay = 60_000`, `fixedDelay = 50`을 설정한다. 실제 scheduled method의 Spring proxy invocation smoke는 main-source fixture를 `@SpringBootTest` context bean에서 직접 두 번 호출하고 packaged YAML의 5초 lease floor를 그대로 검증하므로 두 호출 합계에 `assertTimeout(Duration.ofSeconds(15))`를 적용한다. scheduler trigger와 edge-case runner는 `min-lease-time=0s`를 사용한다. 이 direct-call smoke는 leader aspect observation만 검증하고 Spring scheduler wrapper의 Observation을 검증한다고 표현하지 않는다. pending task 취소는 별도 lifecycle assertion으로 검증한다. `Thread.sleep` 기반 대기는 사용하지 않고 `CountDownLatch`와 `Duration.ofSeconds(5)` 이상의 bounded timeout을 사용하며, in-flight task가 context close로 interrupt된다고 가정하지 않는다. upstream `LeaderElectionAspect`의 target/method metadata cache와 immutable registry는 그대로 소비하며, consumer 예제가 매 tick마다 reflection scan·selector parsing·factory lookup을 새로 수행하지 않는다는 비회귀 근거를 테스트 설명에 남긴다.
...
6. **Spring AOP opt-out 오해**: `spring.aop.auto=false`는 profile configuration의 명시적인 `@EnableAspectJAutoProxy`까지 끄지 않지만 `bluetape4k.leader.aop.enabled=false`는 factory/registry/aspect를 조건부로 끌 수 있다. 두 설정의 task·registry·proxy invocation 상태와 README 경고를 함께 검증한다.
...
- [x] BOM 기반 versionless `bluetape4k-leader-spring-boot`·`bluetape4k-leader-micrometer` dependency가 compile/test classpath에 제공된다.
- [x] `scheduled-policy` profile configuration이 `@EnableAspectJAutoProxy(proxyTargetClass = true)`로 upstream aspect를 runtime proxy하고, Boot의 자동 proxy creator와 중복되지 않는다.
- [x] `scheduled-policy` profile을 명시하지 않은 default context에서 registry/BPP와 `tenantScheduledPolicyFixture` background task가 생성되지 않는다. 기존 retention task는 별도 기능이므로 제외한다.
- [x] profile YAML의 `tenantScheduledPolicyFixture#reconcile`가 exact selector로 binding되고 task가 정확히 하나 등록된다.
- [x] profile main fixture가 `open` class/method로 Spring proxy 대상이 되고, 실제 proxy invocation·leader-aspect observation·`localLeaderElectionFactory` 선택을 `@SpringBootTest`가 확인한다. 이 smoke는 packaged YAML의 `min-lease-time=5s`를 그대로 사용하고 두 호출 합계에 15초 실행 상한을 둔다. scheduler-level Observation은 별도 lifecycle/trigger acceptance로 분리한다.
- [x] `@LeaderElection`, `@LeaderGroupElection`, `@LeaderScheduled` 명시 annotation이 property policy보다 우선한다.
- [x] empty, malformed, duplicate, unmatched, overload 설정과 plain policy의 invalid duration이 startup에서 실패하고, explicit annotation의 미사용 property semantic은 annotation validator 범위와 구분한다.
- [x] context close 후 pending Spring scheduled task가 취소되고 custom thread/executor가 남지 않으며, in-flight body 중단은 보장하지 않는다는 경계를 검증한다.
- [x] `spring.aop.auto=false`에서도 profile의 명시적인 Spring proxy 경로가 유지되고, `bluetape4k.leader.aop.enabled=false`에서는 factory/registry/aspect가 조건부로 빠져 task가 무잠금 실행될 수 있음을 확인했으며, `FAIL_OPEN_RUN`은 멱등 작업 전용 trusted override로 문서화했다.
- [x] profile의 static bounded non-PII name, `allow-method-invocation=false`, lock-name metric `REDACT` 설정을 테스트로 고정했다.
- [x] 기존 reducer와 README snippet을 포함한 module 39개 test가 변경 없이 통과했다.
- [x] 영어/한국어 README가 profile 명령, YAML 키, exact selector, task 수, precedence, fail-fast, lifecycle, local-only 제한, PII 경고와 failure-mode 표를 같은 사실로 설명하고 parity를 통과했다.
- [x] README에 `Started TenantSchedulerLabAppKt` 성공 신호, 첫 실행 전 최대 60초 대기·min lease 추가 지연, `Ctrl-C` 종료와 deterministic test 명령/기대 결과를 안내했다.
- [x] `scripts/smoke-validate.sh stale-check`가 profile YAML·main configuration·selector·양쪽 README 실행 명령 drift를 통과시키고, `@SpringBootTest` profile startup/close가 packaged resource와 main class를 검증했다.
- [x] smoke/full workflow, stale-check, catalog/BOM, diff check, `:leader-tenant-scheduler:test`, compile 검증 결과를 lesson에 문서화했다.
...
설계는 2026-08-31 사용자가 승인했다. 구현 단계에서 upstream 시험판의 external CTW singleton 경로가 `NoAspectBoundException`으로 실행되지 않는다는 증거를 추가로 확보했으므로, 이 문서의 CTW 제안은 runtime Spring proxy 경로로 대체했다. 이 consumer 예제는 그 경계를 명시하고 upstream artifact 자체의 수정은 Issue #869 범위에 포함하지 않는다.
