...
- Kotlin/동시성: 요청당 `snapshot()`은 한 번만 호출한다. candidate build는 lock
  밖에서 끝내고 immutable value만 publish한다. `runBlocking`은 추가하지 않는다.
...
- 운영: revision, operation, count만 구조화된 metadata와 로그로 확인한다.
...
| search index/query generation 불일치 | 부분 provider reload 금지, 완성된 index generation 전체 교체 |
...
| mutable collection·민감 로그 | copy-on-write materialization, metadata-only 로그 |
