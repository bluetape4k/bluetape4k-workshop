...
|---|---|---|---|
| 1 | Correctness | PASS | Tests cover SNS mapping, SQS ack, retry, dead-letter, malformed payload retry, validation, and cancellation rethrow. |
...
| 4 | Operability | PASS | Smoke script and Examples workflow include the new local-first module, path filters, comments, and artifact paths. |
| 5 | Learner documentation | PASS | English/Korean READMEs explain local-first behavior, SNS versus SQS responsibilities, retry/dead-letter outcomes, and test walkthroughs. |
...
and `malformed SQS payload requests retry without invoking handler()` locks that
behavior.
...
| Gate | Evidence |
|---|---|
| Targeted compile/test | `./gradlew :aws-sqs-sns-coroutines:compileKotlin :aws-sqs-sns-coroutines:compileTestKotlin :aws-sqs-sns-coroutines:test --tests '*OrderNotificationMessagingServiceTest' --warning-mode all --max-workers=1 --console=plain` -> 7 tests passed, `BUILD SUCCESSFUL`. |
| Re-run test | `./gradlew :aws-sqs-sns-coroutines:test --no-build-cache --rerun-tasks --max-workers=1 --console=plain` -> 7 tests passed, `BUILD SUCCESSFUL`. |
