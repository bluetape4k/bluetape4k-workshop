...
- 모듈: `:kotlin-flow-extensions-event-aggregation`
- 브랜치: `refactor/flow-event-aggregation-ecosystem-patterns`
- 리뷰 유형: 7-tier 코드 리뷰와 bluetape4k 생태계 사용 점검
- 초점: audit entry 도메인 경계를 bluetape4k 검증 helper로 조이고, sanitized audit output 계약은 유지했는지 검토했다.
...
- Predicate-heavy token normalization은 복합 ASCII/control-character 정책을 더 명확히 표현하는 bluetape4k helper가 없어 direct `require` call을 유지한다.
- `OrderState.apply`은 generic scalar validation이 아니라 projection invariant를 문서화하므로 explicit same-order guard를 유지한다.
