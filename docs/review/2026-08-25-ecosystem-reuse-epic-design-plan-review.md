...
reviewed implementation commit은 `c5dd0d8c771b06fb89165ed80596952766d93e3f`이며,
...
| 2. 계약·자료구조 | PASS | fixed node는 `reviewed-ancestor`만 허용하고, active child는 active parent의 `reviewed_implementation_oid`(또는 exact parent의 `head_oid`)에 결속한다. 단일 review marker, base→ancestor→head 이력, 문서-only tail, follow-up `exact`/`rebase-aware` 필드를 검증한다. | hosted read-back 필요 |
...
| 6. 테스트·운영 | PASS (로컬) | checker 67개 테스트, 실제 Git history 회귀, JSON/checker/diff 검증과 root `detekt` 106 actionable tasks를 통과했다. | marker tail 이후 hosted Ecosystem/Examples/CI 재실행 |
...
<!-- reviewed_implementation_oid: c5dd0d8c771b06fb89165ed80596952766d93e3f -->
...
`parent_oid=P0.reviewed_implementation_oid`를 사용하고 legacy OID 필드는 계속
`null`로 유지한다.
...
```text
python3 .github/scripts/test_check_ecosystem_reuse.py -q  # 67 tests PASS
...
| 상태 | 필수 증거 | 다음 진입 조건 |
|---|---|---|
| P0 contract PASS | `c5dd0d8c`, checker/manifest/local tests, marker-only evidence tail | hosted gate PASS 및 exact-head read-back |
