...
| Stability | PASS | delayed publication, duplicate, version gap, deterministic transport outage, real broker-path outage, restart, schema evolution, correction 12 tests | 실제 broker failover와 multi-region 복제는 제외 |
...
| Operator/Ops | PASS | outbox state, quarantine snapshot/redrive request audit, route-only rollback guide | retained-source retrieval/republication과 production alert threshold는 조직별 과제 |
| Developer/API | PASS | 독립 module/decoder/repository, raw JDBC guard, serialization/validation/logging architecture guard, BOM-only dependency management | wire schema registry 도입은 제외 |
