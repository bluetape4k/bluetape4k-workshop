...
| Basic | [`kotlin-text-processing`](kotlin/text-processing/) | `tokenizer-core`, `text-search`, `lingua`, `text-korean`, `text-japanese`, `coroutines` | In-memory | 다국어 정규화, 시작 단계 dictionary readiness, 전체 index generation 원자 교체·rollback, 원문 span highlighting |
