...
abstract class AbstractRecommendationSuspendTest {
    ...
    fun `service rejects blank graphName`() {
        assertFailsWith<IllegalArgumentException> {
            RecommendationSuspendService(ops, "")
        }
    }

    @Test
    fun `purchase rejects non-product target endpoint`() = runSuspendIO {
        val alice = service.addUser(USER_ALICE, "Alice")
    ...
    @Test
    fun `purchase rejects missing product endpoint`() = runSuspendIO {
        val alice = service.addUser(USER_ALICE, "Alice")
    ...
    @Test
    fun `follow rejects non-user target endpoint`() = runSuspendIO {
        val alice = service.addUser(USER_ALICE, "Alice")
        val laptop = service.addProduct("laptop", "Laptop")
    ...
    @Test
    fun `ProductRecommendation rejects score and sharedBuyers mismatch`() = runSuspendIO {
    ...
    @Test
    fun `FollowRecommendation rejects count and mutualFollows mismatch`() = runSuspendIO {
        val seed = seedRecommendation(service)
