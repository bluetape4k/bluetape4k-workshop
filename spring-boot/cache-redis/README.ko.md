...
| `AsyncConfig` | 공통 `VirtualThreads` executor를 소유하고 공용 `MdcTaskDecorator`를 Spring adapter에 적용합니다. |
...
사용합니다. 공용 `MdcTaskDecorator`는 immutable caller snapshot과 빈 caller context를 처리하고 성공·오류 뒤
worker의 이전 context를 복원합니다.
