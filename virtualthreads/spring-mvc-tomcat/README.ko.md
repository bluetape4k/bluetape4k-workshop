...
이 예제는 **Spring Boot MVC + Virtual Thread + Embedded Tomcat 예제** 모듈을 실행 가능한 가상 스레드 실행 예제로 보여줍니다. 개발자가 먼저 확인할 경로인 모듈 설정, 샘플 또는 테스트 실행, 반복적인 인프라 코드를 줄이는 라이브러리 또는 프레임워크 API 사용 방식을 중심으로 설명합니다.

## 아키텍처 다이어그램

![Spring Boot MVC + Virtual Thread + Embedded Tomcat 예제 Graphviz 아키텍처 다이어그램](../../docs/images/readme-diagrams/virtualthreads-spring-mvc-tomcat-readme-architecture-01.png)
...
![Spring Boot MVC + Virtual Thread + Embedded Tomcat 예제 아키텍처 다이어그램](../../docs/images/readme-diagrams/virtualthreads-spring-mvc-tomcat-diagram-01.png)

## 흐름 다이어그램

1. `virtualthreads-spring-mvc-tomcat` 예제에 필요한 로컬 런타임을 준비합니다.
