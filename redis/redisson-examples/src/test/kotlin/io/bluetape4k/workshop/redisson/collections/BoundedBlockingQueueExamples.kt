...
class BoundedBlockingQueueExamples: AbstractRedissonTest() {
    ...
    fun `크기 제한이 있는 Queue 사용`() = runTest {
        ...
        repeat(ITEM_SIZE) {
            queue.offerAsync(it + 1).await().shouldBeTrue()
        }

        // 용량 제한으로 요소 추가에 실패한다 
        queue.offerAsync(ITEM_SIZE + 1).await().shouldBeFalse()

        val job = scope.launch {
            // 1초후 요소 1개를 가져온다.
            delay(1000)
            queue.takeAsync().await() shouldBeEqualTo 1
        }
        yield()

        // 요소 [ITEM_SIZE+1]를 추가합니다. (10초간 시도)
        queue.offerAsync(ITEM_SIZE + 1, 10, TimeUnit.SECONDS).await().shouldBeTrue()

        job.join()

        // 요소 [1]은 job 내부에서 가겨갔기 때문에 삭제되었습니다.
        queue.containsAsync(1).await().shouldBeFalse()

        // 요소 [ITEM_SIZE+1]은 추가되었습니다.
        queue.containsAsync(ITEM_SIZE + 1).await().shouldBeTrue()

        // 큐 삭제 
        queue.deleteAsync().await().shouldBeTrue()
    }
}
