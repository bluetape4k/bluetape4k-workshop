...
class LocalCachedMapTest: AbstractRedissonTest() {
    ...
    fun `backCache에 cache item을 추가하면 frontCache 에 반영된다`() = runTest {
        ...
        await atMost 10.seconds.toJavaDuration() untilSuspending { frontCache2.containsKeyAsync(key).await() }
        ...
        await atMost 10.seconds.toJavaDuration() untilSuspending { !frontCache1.containsKeyAsync(key).await() }
    ...
    fun `frontCache1의 cache item을 expire 되면 frontCache2에서도 삭제됩니다`() = runTest(timeout = 30.seconds) {
        ...
        frontCache1.fastPutAsync(keyToEvict, 42).await()
        await atMost 5.seconds.toJavaDuration() until { frontCache2.containsKey(keyToEvict) }

        frontCache2.getAsync(keyToEvict).await() shouldBeEqualTo 42
        ...
        frontCache2.getAsync(keyToEvict).await().shouldBeNull()
    }
}
