...
class TopicExamples: AbstractRedissonTest() {
    ...
    fun `using reliable topic`() = runSuspendIO {
        ...
        // topic 에 메시지 전송
        topic1.publishAsync("message-1").awaitSuspending()
        topic2.publishAsync("message-2").awaitSuspending()

        // topic 에 listener가 2개, 메시지 2개 전송
        await until { receivedCounter.get() >= 2 * 2 }

        topic1.removeAllListenersAsync().awaitSuspending()
        topic2.removeAllListenersAsync().awaitSuspending()
