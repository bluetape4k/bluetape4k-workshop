...
class EcosystemReuseCheckerTest(unittest.TestCase):
    ...
    def test_trusted_manifest_accepts_fresh_planned_ref_replan(self):
        ...
        current["planned_scope_replan_receipts"] = {
            "F1": {"receipt_id": "replan-f1", "checksum": "e" * 64}
        }
        trusted_path = self.root / "trusted.json"
        current_path = self.root / "current.json"
        trusted_path.write_text(json.dumps(trusted), encoding="utf-8")
    ...
    def test_planned_scope_replan_requires_pending_execution_receipt(self):
        ...
        current_path.write_text(json.dumps(current), encoding="utf-8")
        errors = CHECKER.validate_manifest(self.root, current_path, trusted_path=trusted_path)
        self.assertTrue(any("must keep the execution receipt PENDING" in error for error in errors))

    def test_planned_scope_replan_requires_planned_trusted_node(self):
        trusted = self.manifest(state="INVALID", receipt_status="FAIL")
        current = self.manifest()
        current["nodes"][3]["expected_base_ref"] = "develop"
        current["planned_scope_replan_receipts"] = {
            "F1": {"receipt_id": "replan-f1", "checksum": "e" * 64}
        ...
        current_path.write_text(json.dumps(current), encoding="utf-8")
        errors = CHECKER.validate_manifest(self.root, current_path, trusted_path=trusted_path)
        self.assertTrue(any("trusted node to also be PLANNED" in error for error in errors))

    def test_planned_scope_replan_rejects_untrusted_parent_and_base(self):
        trusted = self.manifest()
        trusted["nodes"][3]["parent_track"] = "A1"
        current = self.manifest()
        current["nodes"][3].update({
            "expected_base_ref": "untrusted/ref",
            "parent_track": None,
        })
        current["planned_scope_replan_receipts"] = {
            "F1": {"receipt_id": "replan-f1", "checksum": "e" * 64}
        ...
        current_path.write_text(json.dumps(current), encoding="utf-8")
        errors = CHECKER.validate_manifest(self.root, current_path, trusted_path=trusted_path)
        self.assertTrue(any("cannot clear an existing parent_track" in error for error in errors))
        self.assertTrue(any("must match the repository base or current parent head" in error for error in errors))

    def test_planned_scope_replan_rejects_receipt_without_scope_change(self):
        trusted = self.manifest()
        current = self.manifest()
        current["planned_scope_replan_receipts"] = {
            "F1": {"receipt_id": "replan-f1", "checksum": "e" * 64}
        ...
        current_path.write_text(json.dumps(current), encoding="utf-8")
        errors = CHECKER.validate_manifest(self.root, current_path, trusted_path=trusted_path)
        self.assertTrue(any("changed without an execution scope replan" in error for error in errors))
    ...
    def test_manifest_accepts_rebase_aware_child_scope_after_parent_merge(self):
        manifest = self.manifest_with_follow_up_scope()
    ...
    def test_manifest_rejects_repository_base_policy_for_exact_scope(self):
        manifest = self.manifest_with_follow_up_scope()
        scope = self.follow_up_scope()
        scope["expected_base_ref"] = "develop"
        scope["base_ref_policy"] = "repository-base-after-parent-merge"
        manifest["follow_up_scopes"][0] = scope
        path = self.root / "manifest.json"
        path.write_text(json.dumps(manifest), encoding="utf-8")
        errors = CHECKER.validate_manifest(self.root, path)
        self.assertTrue(any("requires rebase-aware oid_policy" in error for error in errors))

    def test_manifest_rejects_repository_base_policy_for_coordinator_scope(self):
        manifest = self.manifest_with_follow_up_scope()
        scope = self.follow_up_scope(scope_kind="coordinator", oid_policy="rebase-aware")
        scope["expected_base_ref"] = "develop"
        scope["base_ref_policy"] = "repository-base-after-parent-merge"
        manifest["follow_up_scopes"][0] = scope
        path = self.root / "manifest.json"
        path.write_text(json.dumps(manifest), encoding="utf-8")
        errors = CHECKER.validate_manifest(self.root, path)
        self.assertTrue(any("requires child scope_kind" in error for error in errors))

    def test_manifest_rejects_unknown_follow_up_base_ref_policy(self):
        manifest = self.manifest_with_follow_up_scope()
        manifest["follow_up_scopes"][0]["base_ref_policy"] = "floating"
        path = self.root / "manifest.json"
        path.write_text(json.dumps(manifest), encoding="utf-8")
        errors = CHECKER.validate_manifest(self.root, path)
    ...
    def test_manifest_rejects_repository_base_policy_with_parent_ref(self):
        ...
        scope = self.follow_up_scope(oid_policy="rebase-aware")
        scope["base_ref_policy"] = "repository-base-after-parent-merge"
        scope["expected_base_ref"] = "branch/F1"
        manifest["follow_up_scopes"][0] = scope
        path = self.root / "manifest.json"
        path.write_text(json.dumps(manifest), encoding="utf-8")
        errors = CHECKER.validate_manifest(self.root, path)
