...
kover {
    reports.total.xml.onCheck = false
}
...
dependencies {
    ...
    testImplementation(testFixtures(project(":commerce-usage-metering-billing-ledger")))
...
fun Test.verifyExecution() {
    ...
    doLast {
        ...
        val xmlFiles = reports.junitXml.outputLocation.get().asFile.walkTopDown()
...
tasks.register<Test>("integrationTest") {
    description = "Runs PostgreSQL integration tests for event-sourced usage billing."
    group = "verification"
...
tasks.register<Test>("stressTest") {
    description = "Runs event-sourced usage billing stress tests."
    group = "verification"
    testClassesDirs = sourceSets["test"].output.classesDirs
    classpath = sourceSets["test"].runtimeClasspath
    useJava25Runtime()
