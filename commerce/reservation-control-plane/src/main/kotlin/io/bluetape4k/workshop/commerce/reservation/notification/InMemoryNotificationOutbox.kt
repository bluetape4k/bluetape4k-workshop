...
internal class InMemoryNotificationOutbox {
    ...
    fun find(deliveryId: String): NotificationDelivery? =
        lock.withLock {
    ...
    fun markDelivered(
        ...
        finalize(deliveryId, owner, now) { current ->
            current
                .copy(
    ...
    private fun NotificationDelivery.canBeClaimedAt(now: Instant): Boolean =
        when (status) {
