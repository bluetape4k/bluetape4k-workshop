...
internal class ReservationCommandExecutionGate(
    ...
    fun <T> execute(operation: String, rawIdempotencyKey: String, action: () -> T): T {
        val opaqueCommandId = credentials.idempotencyDigest(tenantId, operation, rawIdempotencyKey).take(24)
        return when (val admissionOutcome = admission.execute {
            when (val suppressionOutcome = suppression.execute(opaqueCommandId, action)) {
                ...
                SuppressionOutcome.Suppressed -> throw ReservationCommandException("COMMAND_IN_PROGRESS", null, true)
            }
        }) {
            is AdmissionOutcome.Executed -> admissionOutcome.value.also {
                log.debug { "reservation_command_gate_completed operation=$operation mode=${admissionOutcome.mode}" }
            }
