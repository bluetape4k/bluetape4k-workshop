...
internal object IdempotencyFingerprint {
    ...
    fun request(operation: String, canonicalPayload: String): String =
        digest(REQUEST_DOMAIN, operation, canonicalPayload)

    private fun digest(domain: String, vararg fields: String): String =
        MessageDigest
            .getInstance("SHA-256")
            .digest((sequenceOf(domain) + fields.asSequence()).joinToString("\u0000").toByteArray(StandardCharsets.UTF_8))
