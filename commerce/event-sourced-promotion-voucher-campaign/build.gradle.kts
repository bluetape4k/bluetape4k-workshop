...
springBoot {
    mainClass.set("io.bluetape4k.workshop.commerce.voucher.eventsourced.EventSourcedVoucherApplicationKt")
}
...
fun Test.configureEventSourcedTestRuntime() {
...
tasks.test {
    ...
    useJUnitPlatform {
        excludeTags("integration", "stress")
    }
    failOnZeroTests()
...
tasks.register<Test>("integrationTest") {
    description = "Runs PostgreSQL integration tests for the event-sourced voucher campaign."
    group = "verification"
...
tasks.register<Test>("stressTest") {
    description = "Runs event-sourced voucher campaign stress tests."
    group = "verification"
    testClassesDirs = sourceSets["test"].output.classesDirs
    classpath = sourceSets["test"].runtimeClasspath
    configureEventSourcedTestRuntime()
