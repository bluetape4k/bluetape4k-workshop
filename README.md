...
| Advanced | [`exposed-javers-persistence-audit`](exposed/javers-persistence-audit/) | `javers-core`, `javers-persistence-redis`, `javers-persistence-kafka`, `redisson`, `lettuce`, `testcontainers` | Kafka + Redis (TC) + H2 | Bounded Redisson history, fail-closed Redis head startup, and single-partition Kafka projection to Lettuce |
