...
class BufferedSourceTest: AbstractOkioTest() {
    ...
    interface Factory {
        ...
        companion object {
            ...
            val REAL_BUFFERED_SOURCE: Factory = object: Factory {
                override fun pipe(): Pipe {
                    val buffer = Buffer()
                    return Pipe(toString(), buffer, buffer.asBufferedSource())
                }
            ...
            val ONE_BYTE_AT_A_TIME_BUFFERED_SOURCE: Factory = object: Factory {
                override fun pipe(): Pipe {
                    ...
                    return Pipe(toString(), buffer, bufferedSource)
                }
                    ...
                    val sink = object: ForwardingSink(buffer) {
                        override fun write(source: Buffer, byteCount: Long) {
                            ...
                            for (i in 0 until byteCount) {
                                val box = Buffer()
                                box.write(source, 1L)
                                super.write(box.clone(), 1L)
                            }
                        }
                    }.buffered()

                    return Pipe(toString(), sink, buffer)
                }
            ...
            val PEEK_BUFFER = object: Factory {
                override fun pipe(): Pipe {
                    val buffer = Buffer()
                    return Pipe(toString(), buffer, buffer.peek())
                }
            ...
            val PEEK_BUFFERED_SOURCE = object: Factory {
                override fun pipe(): Pipe {
                    val buffer = Buffer()
                    return Pipe(toString(), buffer, buffer.asBufferedSource().peek())
                }
    ...
    fun pipes(): List<Pipe> = Factory.factories.map { it.pipe() }
