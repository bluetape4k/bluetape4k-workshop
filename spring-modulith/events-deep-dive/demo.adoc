...
<4> Business method completes with exception.
No further events triggered.
...
<4> `ConfigurableTxEventListener` fails.
Publication is not resolved.
<5> `TransactionalListener` succeeds.
Publication is marked as completed.
<6> Application shuts down with one publication left incomplete.
Incomplete publications can be republished on application restart or periodically retried.
