...
class ProductController(private val orderService: OrderService) {
    ...
    @GetMapping
    suspend fun findAll(): List<ProductDTO> = orderService.findAllProducts()

    @GetMapping("/{id}")
    suspend fun findById(@PathVariable id: Long): ProductDTO = orderService.findProductById(id)

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    suspend fun create(@Valid @RequestBody req: CreateProductRequest): ProductDTO =
