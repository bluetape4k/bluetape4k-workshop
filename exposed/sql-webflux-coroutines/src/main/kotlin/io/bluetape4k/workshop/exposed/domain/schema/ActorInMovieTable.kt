...
/**
 * 영화([MovieTable])에 출연한 영화 배우([ActorTable])에 대한 정보를 저장하는 테이블 (Many-to-Many 관계를 표현합니다)
 *
 * ```sql
...
object ActorInMovieTable: Table("actors_in_movies") {
    ...
    val actorId = integer("actor_id").references(ActorTable.id, onDelete = ReferenceOption.CASCADE)
    val movieId = integer("movie_id").references(MovieTable.id, onDelete = ReferenceOption.CASCADE)
