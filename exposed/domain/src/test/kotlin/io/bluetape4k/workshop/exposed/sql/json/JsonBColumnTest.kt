...
class JsonBColumnTest: AbstractExposedJsonTest() {
    ...
    fun `insert and select`(testDB: TestDB) {
        withJsonBTable(testDB) { tester, _, _ ->
            ...
            newResult?.get(tester.jsonBColumn) shouldBeEqualTo newData
        }
    }
    ...
    fun `update json`(testDB: TestDB) {
        withJsonBTable(testDB) { tester, _, data1 ->
            tester.selectAll().single()[tester.jsonBColumn] shouldBeEqualTo data1

            val updatedData = data1.copy(active = false)
            tester.update {
                it[tester.jsonBColumn] = updatedData
            }

            tester.selectAll().single()[tester.jsonBColumn] shouldBeEqualTo updatedData
        }
    }
    ...
    fun `select with slice extract`(testDB: TestDB) {
        ...
        withJsonBTable(testDB) { tester, user1, data1 ->
             ...
             * ```
             */
            val isActive = tester.jsonBColumn.extract<Boolean>("${pathPrefix}active", toScalar = requiresScalar)
             ...
             * ```
             */
            val storedUser = tester.jsonBColumn.extract<User>("${pathPrefix}user", toScalar = requiresScalar)
    ...
    fun `DAO Functions with Json Column`(testDB: TestDB) {
        ...
        val dataTable = JsonTestData.JsonBTable
        val dataEntity = JsonTestData.JsonBEntity
        ...
        withTables(testDB, dataTable) {
            ...
            dataTable.update {
                it[this.jsonBColumn] = updatedJson
            }
             ...
             * ```
             */
            val userTeam = dataTable.jsonBColumn.extract<String>(*path)
    ...
    fun `json contains`(testDB: TestDB) {
        ...
        withJsonBTable(testDB) { tester, user1, data1 ->
            ...
            val newId = tester.insertAndGetId {
                it[this.jsonBColumn] = data1.copy(user = alphaTeamUser)
            }
            ...
            val userIsInAlphaTeam = tester.jsonBColumn.contains(stringLiteral(alphaTreamUserAsJson))
            ...
            if (testDB in TestDB.ALL_MYSQL_LIKE) {
                ...
                val userIsInAlphaTeam2 = tester.jsonBColumn.contains("\"Alpha\"", path = ".user.team")
    ...
    fun `json exists`(testDB: TestDB) {
        ...
        withJsonBTable(testDB) { tester, user1, data1 ->
            ...
            val teamA = "A"
            val newId = tester.insertAndGetId {
                it[this.jsonBColumn] = data1.copy(user = data1.user.copy(team = teamA), logins = maximumLogins)
            }
            ...
            if (testDialect is PostgreSQLDialect) {
                ...
                val hasMaxLogins: Exists = tester.jsonBColumn.exists(filterPath)
                ...
                val isOnTeamA: Exists = tester.jsonBColumn.exists(jsonPath, optional = optionalArg)
    ...
    fun `jsonb with upsert`(testDB: TestDB) {
        ...
        withJsonBTable(testDB) { tester, _, _ ->
            ...
            tester.upsert {
                it[tester.id] = newId
                it[this.jsonBColumn] = newData2
            }

            val newResult = tester.selectAll().where { tester.id eq newId }.singleOrNull()
            newResult?.get(tester.jsonBColumn) shouldBeEqualTo newData2
        }
    }
    ...
    fun `escaped placeholder in custom operator`(testDB: TestDB) {
        ...
        withJsonBTable(testDB) { tester, _, data1 ->
            ...
            val topLevelKeyResult = tester
                .selectAll()
                .where { tester.jsonBColumn keyExists "logins" }
                .single()
            topLevelKeyResult[tester.jsonBColumn] shouldBeEqualTo data1

            // `.user.name` 이 있지, `.name` 은 없다.
            val nestedKeyResult = tester
                .selectAll()
                .where { tester.jsonBColumn keyExists "name" }
                .toList()
