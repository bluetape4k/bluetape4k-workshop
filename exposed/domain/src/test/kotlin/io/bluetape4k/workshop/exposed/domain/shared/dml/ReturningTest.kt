...
/**
 * `insertReturning`, `upsertReturning`, `upsertReturing`, `deleteReturning` 구문을 테스트합니다.
 *
...
class ReturningTest: AbstractExposedTest() {
    ...
    /**
     * `insertReturning` 은 Postgres, MariaDB 에서만 지원
     */
    @ParameterizedTest
    ...
    fun `insert ignore returning`(testDB: TestDB) {
        ...
        withTables(testDB, tester) {
             ...
             * ```
             */
            val resultWithoutConflict = tester.insertReturning(ignoreErrors = false) {
                it[item] = "Item B"
            }.single()
    ...
    /**
     * `upsertReturning` 은 Postgres, SQLite 에서만 지원
     */
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    ...
    fun `upsert returning with DAO`(testDB: TestDB) {
        ...
        withTables(testDB, Items) {
            ...
            /**
             * ID가 1인 레코드가 이미 존재하므로, 해당 레코드만 업데이트합니다.
             *
             * ```sql
             * INSERT INTO items (id, "name", price) VALUES (1, 'B', 200.0)
             * ON CONFLICT (id) DO
    ...
    /**
     * [updateReturning] 은 Postgres, SQLite 에서만 지원
     *
