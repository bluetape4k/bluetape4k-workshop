...
class UpsertTest: AbstractExposedTest() {
    ...
    fun `upsert with manual update using insert values`(testDB: TestDB) {
        ...
        withTables(testDB, tester) {
             ...
             *      UPDATE `name`=CONCAT(tester.`name`, ' || ', NEW.`count`), `count`=(1 + NEW.`count`)
             * ```
             * Postgres:
             * ```sql
             * INSERT INTO tester (id, "name", "count") VALUES (2, 'Word B', 2)
    ...
    fun `Inserted Count With BatchUpsert`(testDB: TestDB) {
        ...
        withTables(testDB, AutoIncTable) {
             ...
             * Postgres:
             * ```sql
             * INSERT INTO auto_inc_table (id, "name") VALUES (1, 'A') ON CONFLICT (id) DO UPDATE SET "name"=EXCLUDED."name"
             * ```
             */
    ...
    fun `Upsert With UUID PrimaryKey`(testDB: TestDB) {
        ...
        withTables(testDB, tester) {
             ...
             *      UPDATE SET test_value=EXCLUDED.test_value  -- id 는 업데이트 되지 않음
             * ```
             */
    ...
    fun `Upsert When ColumnName Includes TableName`(testDB: TestDB) {
         ...
         *      my_table_value VARCHAR(100) NOT NULL
         * )
         * ```
         */
        ...
        withTables(testDB, tester) {
             ...
             * VALUES (1, 'Hello')
             * ON CONFLICT (my_table_id) DO
             *      UPDATE SET my_table_value=EXCLUDED.my_table_value
             * ```
             */
            tester.upsert {
                it[myTableId] = 1
    ...
    fun `Default Values And Nullable Columns Not In Arguments`(testDB: TestDB) {
         ...
         *      "defaultExpression" VARCHAR(128) DEFAULT 'defaultExpression' NOT NULL,
         *      "nullable" VARCHAR(128) NULL,
         *      "nullableDefaultNull" VARCHAR(128) DEFAULT NULL NULL,
         *      "nullableDefaultNotNull" VARCHAR(128) DEFAULT 'nullableDefaultNotNull' NULL,
         *      "databaseGenerated" INT DEFAULT 1 NOT NULL
         * )
         * ```
         */
        val tester = object: UUIDTable("tester") {
        ...
        val testerWithFakeDefaults = object: UUIDTable("tester") {
            val number = integer("number")
        ...
        withTables(testDB, tester) {
             ...
             *
             * ```sql
             * INSERT INTO tester ("number", id)
             * VALUES (10, '791819c7-c34f-4c6d-b7b1-7b6a04941a5e')
             * ON CONFLICT (id) DO
