...
이 예제는 **kotlin/text-processing** 모듈을 실행 가능한 Kotlin 언어와 코루틴 패턴 예제로 보여줍니다. 개발자가 먼저 확인할 경로인 모듈 설정, 샘플 또는 테스트 실행, 반복적인 인프라 코드를 줄이는 라이브러리 또는 프레임워크 API 사용 방식을 중심으로 설명합니다.

![kotlin/text-processing 시나리오 다이어그램](../../docs/images/readme-diagrams/kotlin-text-processing-scenario-01.png)

## 아키텍처 다이어그램

모듈은 샘플 진입점 또는 테스트 픽스처, bluetape4k 확장 계층, 예제가 사용하는 런타임 의존성으로 구성됩니다. README와 코드를 비교할 때는 `io.bluetape4k.workshop.kotlin` 패키지 아래의 구현을 기준으로 삼습니다.

## 흐름 다이어그램

1. `kotlin-text-processing` 예제에 필요한 로컬 런타임을 준비합니다.
